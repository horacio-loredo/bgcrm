														...
														<div class="row">
															<div class="col-sm-3">
																...
																<select class="custom-select" id='nivel1'>
																	...
																	$query_n1 = "SELECT * FROM Biggestion.NIVEL1 WHERE ID_CAMP = '3'";
																	$resultado_n1 = $mysqli->query( $query_n1 );

																	while ( $row = $resultado_n1->fetch_assoc() ) {
																		?>
																		<option value="<?php echo $row['ID_NIVEL1']; ?>">
																			<?php echo $row['NOM_NIVEL1']; ?>
																		</option>
																	<?php } ?>
																</select>
...
<script>
	function cerrar() {
		$.ajax( {
			...
			location.href = 'http://172.18.55.18/biggestioncrm/vistas/login.php';
		} );
	}
</script>
...
<?php
	...
	header( "location: http://172.18.55.18/biggestioncrm/vistas/login.php" );
}
?>